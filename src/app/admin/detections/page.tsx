// src/app/admin/detections/page.tsx
...
export default function DetectionDetailsPage() {
  ...
  const fetchDetectionDetails = async () => {
    try {
      ...
      if (!params?.id) {
        setError("Detection ID is missing. Please check the URL.");
        setLoading(false);
        return;
      }

      if (fetchError) throw fetchError;
      if (!data) throw new Error("Detection not found");
  ...
  useEffect(() => {
    if (!detection || !detection.latitude || !detection.longitude || mapLoaded)
      return;
    ...
    const loadMap = async () => {
      try {
        ...
        L.marker([detection.latitude!, detection.longitude!])
          .addTo(map)
          .bindPopup(
            `<b>${detection.species || "Snake Detection"}</b><br>${formatDate(
              detection.timestamp
            )}`
          )
          .openPopup();
            ...
            <div className="p-4">
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  ...
                  <div
                    className={`text-sm rounded-md px-3 py-2 inline-flex items-center ${confidenceClasses[confidenceLevel]} border`}
                  >
                ...
                <div>
                  ...
                  {editMode ? (
                    <select
                      value={status}
                      onChange={(e) =>
                        setStatus(e.target.value as DetectionStatus)
                      }
                    ...
                    <div
                      className={`text-sm rounded-md px-3 py-2 inline-flex items-center ${
                        statusColors[detection.status || "pending"]
                      } border`}
                    >
                      {status === "captured" ? (
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        Are you sure you want to delete this detection? This
